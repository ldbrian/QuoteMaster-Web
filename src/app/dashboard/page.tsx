...
import { 
  ...
  Gift, Crown, Sparkles, 
  Phone, UploadCloud, UserPlus, ChevronRight, CheckCircle2, Copy // 🌟 CTO 新增：任务中心专用图标
} from 'lucide-react'; 
            ...
            <div className="p-6 space-y-4 bg-slate-50">
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between hover:border-blue-300 transition-colors shadow-sm">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center shrink-0"><Phone size={20} /></div>
                  <div>
                ...
                <div className="text-right flex flex-col items-end gap-2">
                  ...
                  <button onClick={() => alert("📱 手机号绑定模块正在接入短信供应商，预计下周上线！")} className="text-xs bg-slate-900 hover:bg-slate-800 text-white px-4 py-1.5 rounded-lg font-medium transition-colors">去绑定</button>
                </div>
              </div>
  ...
  return (
    <tr onClick={onClick} className={`transition-colors group ${isAnalyzing ? 'cursor-wait opacity-80' : 'hover:bg-slate-50/80 cursor-pointer'}`}>
      ...
      <td className="px-6 py-4 hidden md:table-cell">
        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-500 uppercase tracking-wide"><Globe size={10} className="mr-1" /> {region}</span>
      </td>
      <td className="px-6 py-4">
        <span className={`inline-flex px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide ${(statusMap as any)[status] || 'bg-slate-100 text-slate-500'} flex items-center gap-1`}>
          {isAnalyzing && <Loader2 size={10} className="animate-spin" />}{statusTextMap[status] || status}
        </span>
      </td>
      <td className="px-6 py-4 text-sm font-bold text-slate-900">{price}</td>
      <td className="px-6 py-4 text-right">
        <div className="flex items-center justify-end gap-3">
          {isFailed ? <button onClick={onRetry} className="text-xs px-3 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-100 hover:text-rose-700 rounded-md font-medium transition-colors">重新测算</button> : <button className="p-1.5 text-slate-300 hover:text-blue-600 hover:bg-slate-100 rounded-md transition-colors" title="更多操作"><MoreVertical size={16} /></button>}
          <button onClick={onDelete} className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors" title="删除记录"><Trash2 size={16} /></button>
        </div>
      </td>
    </tr>
  );
