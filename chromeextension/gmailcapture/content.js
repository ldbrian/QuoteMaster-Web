(() => {
  ...
  function ensurePanel() {
    ...
    panel.querySelector("[data-qm-capture]").addEventListener("click", async () => {
      const payload = await captureCurrentGmailMessage("manual-ui");
      if (!payload) {
        updatePanel({
          ok: false,
  ...
  async function captureCurrentGmailMessage(source = "auto") {
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          updatePanel({
            ok: false,
            title: "发送失败",
            detail: chrome.runtime.lastError.message || "无法连接插件后台。",
            capturedData,
          });
          return;
        }
        ...
        if (response?.ok) {
          const threadId = response.data?.thread_id || response.data?.data?.thread_id || "";
          updatePanel({
            ok: true,
            title: "已写入业务线程",
            detail: threadId ? `线程 ID：${threadId}` : "邮件已成功发送到 QuoteMaster 后端。",
            capturedData,
          });
          return;
        }
        ...
        updatePanel({
          ok: false,
          title: "后端写入失败",
          detail: response?.error || "QuoteMaster 后端未返回成功状态。",
          capturedData,
        });
      }
    );
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    captureCurrentGmailMessage("manual").then((payload) => {
